...
int read_person(FILE *fin, staff_array *staff, uint16_t i, uint16_t job_count) {
    ...
    if (args_read != 1 || (staff->age[i] < MIN_AGE) || (staff->age[i] > MAX_AGE)) {
        printf("5\n");
        free(staff->name[i]);
        free(staff->surname[i]);
        free_people(staff);
...
void write_jobs(FILE *fout) {
    ...
    char *jobnames[JOBS] = {"программист", "инженер", "продавец", "грузчик", "плотник", "менеджер", "блоггер",
                            "бизнесмен", "госслужащий", "спортсмен"};
...
void write_random_people(staff_array* staff, FILE *fout) {
    ...
    char *male_surnames[NAMES] = {"Иванов", "Петров", "Сидоров", "Измайлов", "Макаров", "Алексеев", "Павлов"};
    char *male_names[NAMES] = {"Борис", "Глеб", "Иван", "Петр", "Артём", "Евгений", "Дмитрий"};
    char *female_surnames[NAMES] = {"Кузнецова", "Разина", "Тихонова", "Третьякова", "Пушкина", "Добрынина",
                                    "Боголюбова"};
    char *female_names[NAMES] = {"Анна", "Юлия", "Екатерина", "Надежда", "Дарья", "Софья", "Елизавета"};
